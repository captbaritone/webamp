...
const imageSelectors = {
  ...
  EQ_SLIDER_THUMB: ['.band input::-webkit-slider-thumb', 'band input::-moz-range-thumb'],
  EQ_SLIDER_THUMB_SELECTED: ['band input::-webkit-slider-thumb:active', '.band input::-moz-range-thumb:active'],
  EQ_ON_BUTTON: ['#on'],
  ...
  MAIN_OPTIONS_BUTTON_DEPRESSED: ['#title-bar #option:active', '#title-bar #option:selected'],
  ...
  MAIN_VOLUME_THUMB: ['#volume::-webkit-slider-thumb', '#volume::-moz-range-thumb'],
  MAIN_VOLUME_THUMB_SELECTED: ['#volume::-webkit-slider-thumb:active', '#volume::-moz-range-thumb:active']
};
  ...
  Object.keys(imageSelectors).map((imageName) => {
    ...
    if (imageUrl) {
      imageSelectors[imageName].forEach((selector) => {
        cssRules.push(`#winamp2-js ${selector} {background-image: url(${imageUrl})}`);
      });
    }
  });
