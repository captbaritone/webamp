...
class ClipPaths extends React.Component {
  ...
  render() {
    // this.props.children should be an object containing arrays of strings. The keys are ids, and the arrays are arrays of polygon point strings
...
const Skin = props => {
  ...
  cssRules.push(
    "#winamp2-js #main-window { clip-path: url(#mainWindowClipPath); }",
    "#winamp2-js #main-window.shade { clip-path: url(#shadeMainWindowClipPath); }",
    "#winamp2-js #equalizer-window { clip-path: url(#equalizerWindowClipPath); }",
    "#winamp2-js #equalizer-window.shade { clip-path: url(#shadeEqualizerWindowClipPath); }"
  );
  ...
  const clipPaths = {};
  Object.keys(mapRegionNamesToIds).forEach(key => {
    const polygons = props.skinRegion[key];
    if (polygons) {
      const id = mapRegionNamesToIds[key];
