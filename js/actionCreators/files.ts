...
export function fetchMediaTags(file: string | Blob, id: number): Dispatchable {
  return async (dispatch, getState, { requireMusicMetadata }) => {
    ...
    dispatch({
      type: SET_MEDIA,
      kbps: metadata.format.bitrate ? (Math.round(metadata.format.bitrate / 1000).toString()) : '128',
      khz: metadata.format.sampleRate? (Math.round(metadata.format.sampleRate / 1000).toString()) : '41',
      channels: metadata.format.numberOfChannels ? metadata.format.numberOfChannels : 1,
      length: metadata.format.duration ? metadata.format.duration : 0,
      id
    });
  };
}
