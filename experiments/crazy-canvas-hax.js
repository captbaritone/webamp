    ...
    function enable(){
        ...
        var animate_fns = Array.from(windows).map((window_el)=> {
            ...
            return ()=> {
                ...
                if(canvas.width !== window_el.clientWidth || canvas.height !== window_el.clientHeight){
                ...
                Array.from(stuff).reverse().forEach((el)=> {
                    ...
                    ctx.save();
                    ctx.translate(x, y);
                    ctx.drawImage(wrappyCanvas, 0, 0, width, height, 0, 0, width, height);
                    ctx.restore();
                });
            };
        });
        ...
        window.hax_cleanup = ()=> {
            Array.from(document.querySelectorAll(".hacky-canvas")).forEach((el)=> { el.remove(); });
