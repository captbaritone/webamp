(function(window, undefined) {
	...
	function setNodeToString(node, string) {
		stringElement = _stringNode(string);
		node.innerHTML = '';
		node.appendChild(stringElement);
	}

	// Get a <div> containing char
	function characterNode(char) {
	...
	function displayCharacterInNode(character, node) {
		position = _getCharPosition(character);
		row = position[0];
		column = position[1];
	...
	function _stringNode(string) {
		parentDiv = document.createElement('div');
		for (var i = 0, len = string.length; i < len; i++) {
			char = string[i].toLowerCase();
			parentDiv.appendChild(characterNode(char));
		}
		return parentDiv;
	}

	// Find the background offsets for a given character
	function _getCharPosition(char) {
	...
	var self = {
		setNodeToString: setNodeToString,
		characterNode: characterNode,
		displayCharacterInNode: displayCharacterInNode,
		digitNode: digitNode
	}

	// There can be only one....
	if (!window.Font) window.Font = self;
